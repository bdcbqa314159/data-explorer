...
pub struct App {
    // API base: "" for the browser (relative), "http://127.0.0.1:PORT" for desktop.
    base: String,
    watchlist: Watchlist,
    cache: SeriesCache,
...
impl App {
    pub fn new(cc: &eframe::CreationContext<'_>, base: String) -> Self {
        let app = Self {
            base,
    ...
    fn reload_watchlist(&self) {
        ...
        ehttp::fetch(ehttp::Request::get(format!("{}/api/watchlist", self.base)), move |result| {
    ...
    fn do_search(&self, query: &str) {
        *self.search.lock().unwrap() = SearchState::Loading;
        ...
        let url = format!("{}/api/search?q={}", self.base, url_encode(query));
    ...
    fn add_signal(&self, id: &str) {
        ...
        let mut req = ehttp::Request::post(format!("{}/api/watchlist", self.base), body);
    ...
    fn ensure_series(&self, id: &str) {
        ...
        let url = format!("{}/api/series/{id}", self.base);
        ehttp::fetch(ehttp::Request::get(url), move |result| {
            ...
            .show(ui, |ui| {
                ...
                match &*wl {
                    ...
                    Some(Ok(items)) if items.is_empty() => {
                        ui.label("No signals — click ＋ Add to search FRED.");
                    }
                    Some(Ok(items)) => {
